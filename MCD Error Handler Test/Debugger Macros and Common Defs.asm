...
;		use "sym|split", so the displacement won't be displayed until symdsp is met
;	* The "symdsp" can only be used after the "sym|split" instance, which decodes offset, otherwise, it'll
;		display a garbage offset.
;	* No other argument format flags (hex, dec, bin, str) are allowed between "sym|split" and "symdsp",
;		otherwise, the "symdsp" results are undefined.
...
split	equ		8				; DO NOT write displacement (if present), skip and wait for "symdsp" flag to write it later (optional)
forced	equ		4				; display "<unknown>" if symbol was not found, otherwise, plain offset is displayed by the displacement formatter

; ... for symbol displacement formatter (symdsp)
